plotdata <- read.csv("results_odds.tsv", header = TRUE, sep = "\t")
...
getQuantiles <- function(plotdata, col) {
  ret <- lapply(list(1,2,5,20), function(nsites) {
      ret <- lapply(list("DBM", "GAN", "IM", "MICE", "VAE"), function(modeltype) {
...
pdf(file = "ordist.pdf", width = 9, height = 8)
plotORdist(plotdata)
dev.off()

pdf(file = "overfitting.pdf", width = 9, height = 8)
